...
def test_publication_persistence_api_round_trip(monkeypatch, tmp_path: Path) -> None:
    ...
    fetched_package = client.get(f"/api/v1/civicdata/ckan-package/{package_id}")
    ...
    fetched_plan = client.get(f"/api/v1/civicdata/publication-plan/{plan_id}")
...
def test_get_ckan_package_without_persistence_returns_actionable_503(monkeypatch) -> None:
    ...
    assert detail["message"] == "CivicData publication workpaper persistence is not configured."
    assert "Set CIVICDATA_PUBLICATION_DB_URL" in detail["fix"]
...
def test_get_publication_plan_missing_id_returns_actionable_404(
    ...
    response = client.get("/api/v1/civicdata/publication-plan/missing")
